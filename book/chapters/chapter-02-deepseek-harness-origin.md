...
## 2.2 崔添翼其人：量化、Koishi 与 Harness 的同构性

2026 年 3 月，也就是招聘启事发布前两个月，Harness 团队的负责人已经就位：**崔添翼**（Cui Tianyi），浙大计算机系毕业，在量化交易巨头 **Jane Street 任职九年**，2026 年 3 月加入 DeepSeek 组建 Harness 团队（东方财富，2026-08-13）。
...
但这只是崔添翼的一半背景。另一半背景，对本书读者来说更加重要：**他同时是 Cordis 上游生态（Koishi / cordiverse，GitHub ID：Shigma）的作者**。

如果你混过中文开源社区，大概率听说过 Koishi——一个基于插件框架的跨平台聊天机器人框架，经年累月积累了广泛的用户基础，核心思想就是"一切皆插件"。而 Cordis，正是从 Koishi 项目中沉淀、抽象出来的通用插件框架（github.com/cordiverse/cordis）。

现在把两块拼图合起来：

- dsh 的官方定位写着 **"powered by Cordis"**——整个 Harness 建立在这个插件框架之上；
- 而 dsh 团队的负责人，恰好是 Cordis 的作者本人。

这解释了 DeepSeek Harness 技术选型上最大的"意外"：一家 AI 公司的 Agent Harness，没有从零自研框架，也没有选用社区里更"AI 原生"的方案，而是建在一个人畜无害的 TypeScript 插件框架之上。这不是拍脑袋，这是**作者带着自己的框架来执掌团队**——Cordis 在 Koishi 生态经过了经年累月的实战检验，其生命周期管理、依赖注入、配置热重载的成熟度，远非一个三个月的新框架可比。
...
**其三，8 月 13 日的"同日开源"是一次精心编排的双响炮。** Harness v0.1 与旗舰模型 V4-Pro 同日发布，传递的信号再明确不过：模型与 Harness 从此是 DeepSeek 的两条同等重要的产品线，互为犄角。开源后的社区响应验证了这个策略——**几天之内，社区就产出了约 300 个插件**。对于一个以"一切皆插件"为核心理念的项目来说，没有什么比插件生态的爆发更能证明架构的吸引力。
...
第二条力量是安全。2026 年 6 月，Claude Code 被曝"**隐写回传**"事件，工信部 NVDB 随后发布安全后门风险提示；阿里、腾讯、美团、京东等公司相继排查并转向自研 Harness（2026-06）。
...
第三条力量是 DeepSeek 的传统艺能——成本。第三方测试显示，用 DeepSeek V4 实现类似 Claude Code 体验的成本"**仅为几十分之一**"。这个成本优势由两部分构成：更便宜的模型，加上更好的 Harness 优化（缓存对齐、上下文裁剪、渐进式披露——第 1 章的每一条原则都在省钱）。市场也够大：2025 年全球 AI 编程工具市场规模 295.7 亿美元，预计 2030 年达 646.8 亿美元（Research and Markets）。
...
**第二句："Powered by Cordis."** 插件体系的地基是 Cordis 框架（github.com/cordiverse/cordis），由团队负责人崔添翼本人创作、经 Koishi 生态多年检验。本书前半部分（第 3—4 章）将系统讲解 Cordis 的插件模型、生命周期与依赖注入——那是读懂 dsh 一切源码的钥匙。
...
- 团队负责人**崔添翼**：浙大计算机系、Jane Street 九年量化背景（2026-03 加入 DeepSeek）——量化系统"把概率模型翻译成可审计、可回退的确定性执行"与 Harness 工程高度同构；他同时是 **Cordis / Koishi 生态的作者（Shigma）**，这直接决定了 dsh 的技术底座，也为本书"从 Cordis 讲起"埋下伏笔。
- 7–8 月闪电推进：公众号注册（07-06）→ V4-Flash 公测剧透"Harness 极简模式"（07-31）→ 全球内测招募（08-01，769 人 / 712 仓库 / 120 万+ Stars / 18 赛道）→ **v0.1 与 V4-Pro 同日 MIT 开源（08-13）** → 数日内社区产出约 300 个插件。
